...
{
//	uint8_t i;
	memcpy(&ota_img_header, (void *)OTA_TEST_IMG, sizeof(ota_img_header_s));
	...
	W25qxx_Init();
//	printf("Erase external flash OTA block");
//	for (i = 0; i < 16; i++){
...
void app_ota_write_raw_img(void)
{
	...
	while(img_offset < OTA_TEST_IMG_LEN) {
		if(ota_page_idx == OTA_IMG_FIRST_PAGE_IDX) {
			...
			W25qxx_WritePage(&OTA_TEST_IMG[img_offset], ota_page_idx, RAW_IMG_FIRST_OFFSET, read_len);
		} else {
			read_len = ((OTA_TEST_IMG_LEN - img_offset) > W25QFLASH_PAGE_SIZE) ? W25QFLASH_PAGE_SIZE : OTA_TEST_IMG_LEN - img_offset;
			W25qxx_WritePage(&OTA_TEST_IMG[img_offset], ota_page_idx, 0, read_len);
		}

		crc32 = hw_crc32_feeddata((uint32_t *)&OTA_TEST_IMG[img_offset], read_len/4);
		img_offset += read_len;
		ota_page_idx++;
		printf(".");
	}
...
//		printf("img_offset : %d\r\n", img_offset);
	...
	if(crc32 == ota_img_header.crc32) {
		// write header
		W25qxx_WritePage(&OTA_TEST_IMG[0], 0, 0, sizeof(ota_img_header_s));
		printf("App OTA Done\r\n");
	} else {
		printf("CRC32 ERROR\r\n");
	}
